...
def inference(
    ...
    for i in range(len(azimuth_angles)):
        x, y, z, description, show_in_im2, output_ims = main_run_inference(models, device, 'angles_gen', float(azimuth_angles[i]), float(elevation_angles[i]), 0.0, raw_im=raw_im)
