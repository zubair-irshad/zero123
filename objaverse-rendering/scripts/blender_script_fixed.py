...
def load_camera_positions(camera_positions_path):
    with open(camera_positions_path, "r") as f:
        camera_positions = json.load(f)
    return camera_positions

def randomize_camera(camera_positions):
    # elevation = random.uniform(0., 90.)
    # azimuth = random.uniform(0., 360)
    # distance = random.uniform(0.8, 1.6)
    # return set_camera_location(elevation, azimuth, distance)
    return set_camera_location(camera_positions[0], camera_positions[1], camera_positions[2])
    

def set_camera_location(x, y, z):
...
def normalize_scene(box_min_max):

    bbox_min = Vector(box_min_max[0])
    bbox_max = Vector(box_min_max[1])
...
def save_images(object_file: str, camera_positions, box_min_max) -> None:
    ...
    for i in range(args.num_images):
        ...
        # set camera
        camera = randomize_camera(camera_positions[i])
...
if __name__ == "__main__":
    try:
        ...
        #load all scene boxes based on 90 degree articulation
        box_file = '/home/zubairirshad/zero123/objaverse-rendering/bbox_90.json'
        ...
        box_min_max = all_bboxes[idx]

        fixed_camera_paths = '/home/zubairirshad/zero123/objaverse-rendering/camera_positions'
        camera_positions = load_camera_positions(os.path.join(fixed_camera_paths, 'positions_partnet_fixed'+ idx +'.json'))
        
        save_images(local_path, camera_positions, box_min_max)
        end_i = time.time()
